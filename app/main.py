...
# el postventa se ejecuta una vez al día.
...
async def _scheduler_loop():
    ...
    while True:
        ...
        try:
            db = SessionLocal()
            try:
                res = await run_reminders(token=settings.admin_token, db=db)
                if res.get("sent"):
        ...
        try:
            ...
            if _last_followups_date != today:
                db = SessionLocal()
                try:
                    res = await run_followups(token=settings.admin_token, db=db)
...
async def lifespan(_app: FastAPI):
    logger.info("Iniciando Vita Qualitat WhatsApp Agent...")
    init_db()
    ...
    logger.info("Planificador interno iniciado (recordatorios + postventa).")
    yield
    task.cancel()
...
app = FastAPI(
    title="Vita Qualitat WhatsApp Agent",
    description="Agente inteligente de WhatsApp para Vita Qualitat",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def security_headers(request, call_next):
    """Cabeceras de seguridad básicas para todo el sitio (panel y webhook)."""
    try:
...
async def health():
    import shutil
    from app import runtime
    return {"status": "ok", "service": "vita-qualitat-bot",
